...
|---|---|---|---|
| **Carte météo + verdict d'arrosage** | ✅ | — | `MeteoAccueilNotifier` (`meteoAccueilProvider`) : températures du jour + **verdict d'arrosage** niveau jardin (pluie à venir / sol humide / arroser / clément, seuils de `BilanArrosage`) depuis `meteoServiceProvider` + la **position du potager actif**. `_CarteMeteo` branché ; invite à renseigner la position si aucune (cf. onboarding §7). |
