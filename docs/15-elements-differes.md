...
| **Version dynamique** | 🔵 codée en dur | `_versionAffichee = '1.0.0'` dans `panneau_a_propos.dart` (miroir manuel de `pubspec.yaml`). Lire la version au runtime = `package_info_plus` (présent en **transitif** seulement, jamais importé — dépendance directe à valider). | Valider `package_info_plus` (100 % local : lit version/build, aucun réseau), l'ajouter au pubspec et remplacer la constante. |
