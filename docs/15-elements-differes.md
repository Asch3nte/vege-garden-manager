...
| # | Élément | État | Ce qui manque / où |
|---|---|---|---|
| 4 | **Panneau « Aide & lexique »** | ⚪ | Nouveau panneau accessible depuis l'onglet **Plus** : définitions détaillées de chaque terme propre au potager + tips. Les mêmes tips s'afficheront **en bulles contextuelles au bon moment**, l'utilisateur pouvant toujours revenir ici. Prévoir un modèle de contenu (terme → définition → tip) réutilisé par les bulles. Lien avec §6 (menu Plus). |
...
| 9 | **Node espèce → liste des variétés** | ✅ | La vue Réseau est une **page scrollable** (`CustomScrollView`) : sélectionner une espèce liste **ses variétés** sous le panneau (`SliverList` de `_LigneVarieteReseau`, dérivé de `varietesDe` passé par `ecran_catalogue.dart`), chacune ouvre sa fiche. |
| 10 | **Scroll bas → réduire la tuile réseau, place aux fiches** | ✅ | La constellation vit dans un **`SliverPersistentHeader` réductible** (`_EnteteReseau`, min ~140 px / max ~52 % hauteur) : scroller la liste réduit l'en-tête et, au-delà d'un seuil, **repasse les nodes en initiale** (drapeau `compact` → `_construireCanvas`, plus de libellés #8). Le pan/zoom reste capté par le `GestureDetector` de la tuile ; le scroll se fait sur la liste (décision dev). |
