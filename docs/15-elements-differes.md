...
| **Version dynamique** | ✅ | — | `package_info_plus` (validée, 100 % local : lit la version/build de l'app, aucun réseau) câblé dans `panneau_a_propos.dart`. |
...
| # | Élément | État | Ce qui manque / où |
|---|---|---|---|
| 4 | **Panneau « Aide & lexique » (Glossaire / manuel d'aide)** | ⚪ | Nouveau panneau accessible depuis l'onglet **Plus** : définitions détaillées de chaque terme propre au potager + tips. Les mêmes tips s'afficheront **en bulles contextuelles au bon moment**, l'utilisateur pouvant toujours revenir ici. Prévoir un modèle de contenu (terme → définition → tip) réutilisé par les bulles. **Modèle partagé avec les mini-tutos par palier d'[ADR-0009](decisions/0009-paliers-experience-divulgation-progressive.md) §4a** (feature → pourquoi → comment) et le teaser de montée de palier. Lien avec §6 (menu Plus). **Le glossaire hébergera des pages dédiées par terme** : noms de **familles botaniques** (`FamilleBotanique`), **maladies/ravageurs** (`Bioagresseur`), et enums métier (sol, climat, rusticité, méthodes…). → voir #4bis. |
| 4bis | **Termes cliquables → pages du glossaire + couleurs** | ⚪ | Quand le glossaire (#4) existera : transformer en **liens** les termes affichés un peu partout — d'abord dans la **section famille de la fiche plante** (`_SectionFamille`, ADR-0006 Lot 4) : nom de famille + chips maladies/ravageurs → leur page dédiée ; puis les autres enums (climat, rusticité, sol, usages, méthodes). **Au design final** : choisir une **charte de couleurs par type de terme** (famille / maladie / ravageur / technique…) pour une accroche et une lisibilité visuelles accrues (cf. [08-design-system](08-design-system.md)). **Repasser aussi sur la mise en forme** de la section famille (hiérarchie visuelle, espacements, éventuels pictos) une fois ces conventions posées. |
