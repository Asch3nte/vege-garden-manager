...
| **4 — Mini-tutos par palier + teaser** | **4a ✅** Mini-tuto **re-consultable** : panneau « Guide des niveaux » (`panneau_niveaux.dart`, route `/plus/niveaux`) — catalogue des features **construites** par palier (titre → **pourquoi** → **comment**), badge « Débloqué » selon le niveau, les features des paliers supérieurs servant de **teaser** statique. Contenu éditorial (`_features` + clés `tuto*`). **Reste** : **4b** teaser **contextuel** (cartes « aperçu » in-situ sur les écrans gatés + CTA d'un tap) ; **affichage auto** du guide à l'atteinte d'un palier (changement de niveau / onboarding). | Lot 2 (+ docs/15 §8 C4) |
