...
void main() {
  ...
  testWidgets('ticking a subtask completes only that one', (tester) async {
    ...
    await tester.tap(find.textContaining('3 tâches'));
    await tester.pumpAndSettle();
    // Completion goes through the row's check button: tapping the row itself
    // opens the task detail (a mis-tap must not tick a crop off).
    await tester.tap(
      find.descendant(
        of: find
            .ancestor(
              of: find.text('Arroser les courgettes'),
